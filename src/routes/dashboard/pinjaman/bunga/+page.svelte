...
<div class="container mx-auto px-4 py-8">
    ...
    <div class="mb-4 p-4 bg-yellow-100 border border-yellow-300 rounded">
        ...
        <button
            on:click={async () => {
                console.log("🧪 Testing bunga-options endpoint directly...");
                try {
                    const response = await axios.get("/bunga-options");
                    console.log(
                        "✅ Raw bunga options response:",
                        response.data,
                    );

                    // Show the transformed data too
                    const rawData = response.data.data || [];
                    ...
                    console.log("🔄 Transformed data preview:", transformed);
                } catch (error) {
                    console.error(
                        "❌ Bunga options test failed:",
                        error.response?.status,
                        error.response?.data,
                    );
                }
            }}
            class="mt-2 ml-2 px-3 py-1 bg-purple-500 text-white rounded text-sm"
        >
            Test Bunga GET
        </button>
        ...
        <button
            on:click={async () => {
                console.log("🧪 Testing bunga-options POST directly...");
                const testData = {
                    nama: "Test Bunga " + Date.now(),
                    persen: 1.5,
                    deskripsi: "Test from debug button",
                };
                try {
                    console.log("📤 Sending POST with data:", testData);
                    console.log(
                    ...
                    console.log(
                        "✅ Bunga POST test successful:",
                        response.data,
                    );
                } catch (error) {
                    console.error(
                        "❌ Bunga POST test failed:",
                        error.response?.status,
                        error.response?.data,
                    );
            ...
            class="mt-2 ml-2 px-3 py-1 bg-red-500 text-white rounded text-sm"
        >
            Test Bunga POST
        </button>
