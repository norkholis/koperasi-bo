<script lang="ts">
    ...
    // Reactive current path - ensures nav highlights update on route change
    $: currentPath = $page.url.pathname;

    // Role-specific dashboard paths that should highlight "Dashboard" nav item
    const dashboardPaths = ["/dashboard", "/dashboard/admin", "/dashboard/member", "/dashboard/super"];

    // Check if a path is active
    function isActive(path: string, current: string): boolean {
        // Dashboard highlights on all role-specific home pages
        if (path === "/dashboard") {
            return dashboardPaths.includes(current);
        }
                        ...
                        <div>
                            <button
                                ...
                                class="w-full flex items-center justify-between gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors duration-150
                                    {isActive('/dashboard/pinjaman', currentPath) ? 'text-teal-400 bg-teal-500/10' : 'text-slate-400 hover:text-slate-200 hover:bg-slate-800'}"
                            >
                                        ...
                                        {#if child.show}
                                            <a
                                                ...
                                                class="block px-3 py-2 rounded-lg text-sm transition-colors duration-150
                                                    {isActive(child.href, currentPath) ? 'text-teal-400 bg-teal-500/10' : 'text-slate-400 hover:text-slate-200 hover:bg-slate-800'}"
                                                on:click={closeSidebar}
                                            >
                                                {child.label}
                                            </a>
                    ...
                    {:else}
                        ...
                        <a
                            ...
                            class="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors duration-150
                                {isActive(item.href || '', currentPath) ? 'text-teal-400 bg-teal-500/10' : 'text-slate-400 hover:text-slate-200 hover:bg-slate-800'}"
                            on:click={closeSidebar}
                        >
